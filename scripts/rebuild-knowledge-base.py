...
function reentryPrompt(a){{return a.handoff_prompt||`Usa el artefacto "${{a.title}}" (${{a.id}}) como contexto local. Revisa su metadata, decide si conviene reusar, refrescar, reemplazar o ignorar, y conserva intención, evidencia, límites, frescura y relaciones.`}}
...
function selectArtifact(id){{selected=artifacts.find(a=>a.id===id);$('prompt').value=reentryPrompt(selected)}}
