...
4. Open `docs/HTML_ADVANTAGE_GATE.md` and prove HTML beats Markdown for this task.
5. Open `docs/ARTIFACT_BUDGET.md` and choose compact, standard, or interactive.
6. Use `docs/pattern-router.md` to select one primary pattern.
7. Open the matching file in `patterns/` and apply its evidence requirements, prompt template, HTML structure, quality checklist, failure modes, and acceptance criteria.
8. Open `docs/HTML_UX_STANDARD.md` and define the artifact's UX contract.
9. Inspect real sources before generation.
10. Build a fact sheet using `docs/fact-sheet-protocol.md`.
11. Separate facts, inferences, unsupported claims, and unknowns.
12. Generate the smallest useful HTML artifact.
13. Apply `docs/QUALITY_BAR.md` before finalizing.
14. Compare against `docs/GOLDEN_OUTPUTS.md` when the artifact is a PR review, visual exploration, or project reentry.
15. Include uncertainty visibly.
16. Apply `docs/HTML_ARTIFACT_CHAINS.md` when the work naturally needs exploration, planning, review, or handoff across multiple artifacts.
17. Apply `docs/SHARE_AND_REENTRY.md` when the artifact may be shared or used by another agent later.
18. Apply `docs/NEXT_ACTION_HANDOFF.md` so the artifact ends with a copy-ready next step.
19. Save the artifact to a reasonable local path when tool access allows it.
