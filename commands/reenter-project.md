...
8. Apply `docs/EXPLAINER_CLARITY.md` so the recap explains what the reader can safely do next.
9. Apply `docs/HTML_ADVANTAGE_GATE.md`; a recap must be more than a prettier README.
10. Define the artifact budget from `docs/ARTIFACT_BUDGET.md`.
11. Define the UX contract from `docs/HTML_UX_STANDARD.md`.
12. Build a fact sheet.
13. Compare against the project reentry reference in `docs/GOLDEN_OUTPUTS.md`.
14. Apply `docs/HTML_ARTIFACT_CHAINS.md` to recommend the next artifact only when it removes real work.
15. Apply `docs/SHARE_AND_REENTRY.md` so another agent can continue from the recap.
16. Apply `docs/NEXT_ACTION_HANDOFF.md` so the reentry path is ready to run.
17. Generate HTML after verification.
